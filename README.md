...
LiveKit Cloud (STT → LLM → TTS, room management)
  ...
  ▼
Agent (this repo)
  ├── 10 @function_tools — call deterministic Python, return human-readable strings
...
- **Tools do the analysis, LLM narrates.** 10 function tools call deterministic Python. The LLM never sees raw JSON — it receives pre-computed summaries. This keeps responses accurate, fast, and testable.
...
```bash
uv run pytest                               # All tests (72 non-LLM + 11 LLM-dependent)
