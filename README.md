...
LiveKit Cloud (STT → LLM → TTS, room management)
  ...
  ▼
Agent (this repo)
  ├── 17 @function_tools — call deterministic Python, return human-readable strings
  ├── _navigate_browser() — RPC to frontend, auto-navigates on data lookups
  ├── ServiceHealth — unified error detection for all remote services (tier-based degradation)
...
- **Tools do the analysis, LLM narrates.** 17 function tools call deterministic Python. The LLM never sees raw JSON — it receives pre-computed summaries. This keeps responses accurate, fast, and testable.
...
- **Service health monitoring.** All remote calls (Supabase, git, avatar, summarizer) are wrapped with `ServiceHealth` for tier-based degradation (CRITICAL/IMPORTANT/OPTIONAL). Degraded service warnings are injected into the LLM's context. Health summary logged at session close.
...
uv run pytest tests/test_service_health.py  # Service health monitor (no API keys needed)
```
